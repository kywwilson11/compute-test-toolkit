...
class TestCxlRetimer:
    ...
    def test_empty_lanes_fails(self):
        # lanes=[] probes zero eyes; all([]) is vacuously True, so the verdict
        # must NOT pass. all_lanes_eye_open is fail-safe False even with a
        # wide-open injected eye (the bug would report ok=True here).
        rt = MockRetimer(part_number="PT6161L", injected_eye_ui=0.32,
                         injected_eye_mv=75.0)
        h = check_cxl_retimer(rt, expect_part="PT6161L", lanes=[])
        assert not h.ok and h.checks["all_lanes_eye_open"] is False
